...
public class PathfinderJob implements Supplier<Queue<Vector2i>> {
    ...
    public PathfinderJob(Settings settings) {
    ...
    public Queue<Vector2i> get() {
        ...
        Node currentNode = new Node(
                initialPosition,
                this.calculateDistanceToEndGoal(initialPosition),
                0,
                this.targetSpacity);
        ...
        while (this.activeNodes.size() > 0) {
            // Investigate the best node we have yet to explore
            ...
            if (currentNode.getDistanceFromEndGoal() == 0) {
                break;  // This node is our path
            }

            this.activeNodes.remove(currentNode);
            this.activeNodes.addAll(this.getAdjacentNodes(currentNode));
        }
    ...
    private Set<Node> getAdjacentNodes(Node parentNode) {
        ...
        for (Vector2i adjustment : POSSIBLE_MOVE_ADJUSTMENTS) {
            ...
            // Is it within the boundaries and a empty space?
            if (this.settings.getBoundaries().isWithinThisRegion(new Vector3(nodePosition.getX(), this.settings.getBoundaries().getPosLesser().getY(), nodePosition.getZ())) && this.isBlockAir(nodePosition)) {
                Node newNode = new Node(
                        parentNode,
                        nodePosition,
                        this.calculateDistanceToEndGoal(nodePosition),
                        parentNode.getStepsTaken() + 1,
                        this.getSpacity(nodePosition));
                ...
                if (this.smallestScores.containsKey(nodePosition)) {
                    // Is this route better than the stored?
