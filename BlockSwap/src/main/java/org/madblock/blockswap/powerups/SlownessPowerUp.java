...
public class SlownessPowerUp extends PowerUp {

    private static int RADIUS = 10;

    public SlownessPowerUp(GameBehavior behaviour, Player player) {
    ...
    public String getDescription() {
        return String.format("Use this to slow players that are within a %s block radius!", RADIUS);
    }
                ...
                if (Math.abs(targetPos.getX() - pos.getX()) < RADIUS && Math.abs(targetPos.getZ() - pos.getZ()) < RADIUS) {
                    player.sendMessage(
                            Utility.generateServerMessage(
                                    "POWERUP",
                                    TextFormat.YELLOW,
                                    this.player.getDisplayName() + TextFormat.GRAY + " slowed you down for " + (tickTime / 20) + " seconds."
                            )
                    );
                    player.addEffect(
                            new Effect(Effect.SLOWNESS, "Slowness", 0, 0, 0)
